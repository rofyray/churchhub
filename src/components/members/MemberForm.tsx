...
import { Department, MemberFormData } from '@/lib/types';
...
export default function MemberForm({
  ...
  const [formData, setFormData] = useState<MemberFormData>({
    ...
    joinedVia: initialData?.joinedVia || '',
    ...
    <form onSubmit={handleSubmit} className="space-y-6">
      ...
      <div className="flex items-center gap-6">
        <div
          ...
          className="w-24 h-24 rounded-full bg-white/10 border-2 border-dashed border-white/20 flex items-center justify-center cursor-pointer hover:border-brand-500/50 transition-colors overflow-hidden"
        >
          {photoPreview ? (
        ...
        <div>
          ...
          <p className="text-xs text-slate-400 mt-1">Click to upload (max 5MB)</p>
        </div>
        <input
          ref={fileInputRef}
          type="file"
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
        <div className="w-full">
          ...
          <div className="flex">
            <span className="inline-flex items-center px-4 bg-white/10 border border-r-0 border-white/10 rounded-l-xl text-text-secondary">
              +233
            </span>
            <input
              type="tel"
              ...
              required
              maxLength={9}
              className="flex-1 h-12 bg-white/5 text-white border border-white/10 rounded-r-xl px-4 placeholder:text-text-placeholder hover:border-white/20 focus:outline-none focus:border-brand-500/50 focus:ring-2 focus:ring-brand-500/20 transition-colors duration-200"
            />
          </div>
        </div>
        <div className="w-full">
